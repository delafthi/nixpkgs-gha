...
## Features
- Automatically check for package updates on a schedule (e.g., Monday and Friday at 2 AM UTC)
- Update multiple packages in parallel using matrix strategy
- Automatically create pull requests to nixpkgs with version updates
- Skip updates if a PR already exists for a package
- Optionally integrate with [nixpkgs-review-gha](https://github.com/Defelo/nixpkgs-review-gha) for automated reviews
...
The default schedule runs updates on Monday and Friday at 2 AM UTC. To change this:
...
### 6. Integrate with nixpkgs-review-gha (optional)
If you have [nixpkgs-review-gha](https://github.com/Defelo/nixpkgs-review-gha) set up, you can automatically trigger reviews:

1. In your fork, go to "Settings" > "Secrets and variables" > "Actions" > "Variables" tab
2. [Create a new repository variable](../../settings/variables/actions/new) with the name `NIXPKGS_REVIEW_GHA_FORK`
3. Set its value to your nixpkgs-review-gha fork (e.g., `your-username/nixpkgs-review-gha`)
4. When manually triggering updates, you can specify this fork in the workflow dispatch input

## Usage

### Automatic Updates (Scheduled)
Once configured, the workflow will automatically run on the schedule defined in `.github/workflows/update.yml` and update all packages listed in the `WATCHING_PACKAGES` variable.
...
4. (Optional) Configure PR behavior:
   - **Force open PR**: Create PR even if one already exists
   - **Open PR**: Automatically create a PR (enabled by default)
5. (Optional) Specify your nixpkgs-review-gha fork for automated reviews
...
| `force-open-pr` | No | `false` | Force opening a PR even if one exists |
| `open-pr` | No | `true` | Automatically open a PR |
| `nixpkgs-review-gha-fork` | No | `""` | Fork for triggering nixpkgs-review-gha |
